...
| `// mcpscan-disable-next-line -- trust me` | Names no rule. A blanket suppression would also silence every rule written after it, which is broader than anyone needs, and broader than this scanner's suppression contract allows. |
...
look exactly like a clean one — the same failure as a scan that found no
files at all and said nothing about it.
