...
2. **Invisible Unicode.** The same characters MCP002 looks for in tool definitions (zero-width space, bidi overrides, tag characters, and the rest of the per-class policy described in [MCP002](MCP002.md)) work the same way in a skill body: present in the bytes, absent from what a rendered view shows.
