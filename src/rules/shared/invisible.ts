 ...
 * Extracted from MCP002 so SKILL001 can apply the exact same classification
 * to a skill's body text instead of reimplementing (and inevitably drifting
 * from) the policy.
 *
...
export function describeCp(cp: number): string {
 ...
 */
export function classify(codepoints: string[]): boolean[] {
 ...
 */
export function buildEvidence(codepoints: string[], flagged: boolean[]): string {
  const n = codepoints.length;
...
export interface InvisibleScanResult {
  codepoints: string[];
  flagged: boolean[];
  hitCount: number;
  /** Unique, first-seen-order human-readable descriptions of the flagged codepoints. */
  descriptions: string[];
}

/** Runs the full classify -> describe pipeline over an arbitrary piece of text. */
export function scanInvisible(text: string): InvisibleScanResult {
  ...
  return { codepoints, flagged, hitCount, descriptions };
}
